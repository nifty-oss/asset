      ...
      accounts: {
        ...
        systemProgram: {
          defaultsTo: k.conditionalDefault('account', 'payer', {
            ifTrue: k.programDefault('systemProgram', '11111111111111111111111111111111'),
          }),
        },
      },
    },
...
// Render JavaScript.
kAsset.accept(
  new k.RenderJavaScriptVisitor(path.join(clientDir, 'asset', 'js', 'src', 'generated'), {
    prettier: require(path.join(clientDir, 'asset', 'js', '.prettierrc.json')),
  }),
);

// Render Rust.
kAsset.accept(
  new k.RenderRustVisitor(path.join(clientDir, 'asset', 'rust', 'src', 'generated'), {
    formatCode: true,
...
kBridge.update(
  new k.SetInstructionAccountDefaultValuesVisitor([
    ...
    },
    {
      ...
      ignoreIfOptional: true,
      ...k.programDefault('niftyAsset', 'AssetGtQBTSgm5s91d1RAQod5JmaZiJDxqsgtqrZud73'),
    },
  ]),
);
...
kBridge.update(
  new k.UpdateInstructionsVisitor({
    create: {
      accounts: {
        asset: {
          defaultsTo: k.resolverDefault('resolveBridgeAsset', [k.dependsOnAccount('mint')]),
        },
      },
      args: {
        version: {
      ...
      args: {
        tokenStandard: {
          ...
          defaultsTo: k.valueDefault(
            k.vEnum('TokenStandard', 'NonFungible', undefined, 'mplTokenMetadata'),
          ),
        },
      },
    },
...
// Render JavaScript.
kBridge.accept(
  new k.RenderJavaScriptVisitor(path.join(clientDir, 'bridge', 'js', 'src', 'generated'), {
